...
describe('currentTimeInMinutes', () => {
	it('returns current time of day in minutes', () => {
		jest.spyOn(Date, 'now').mockImplementation(() => (new Date(2000, 0, 0, 1, 1).getTime()));
...
describe('departureTimeToStr', () => {
	it('shows minutes left, if departure in next ten minutes', () => {
		expect(TimeUtils.departureTimeToStr((TimeUtils.currentTimeInMinutes() + 5)*60))
			.toMatch(/5\smin/);
		expect(TimeUtils.departureTimeToStr(TimeUtils.currentTimeInMinutes()*60))
			.toMatch(/0\smin/);

		const mock = jest.spyOn(TimeUtils, 'currentTimeInMinutes').mockImplementation(() => 23*60+59);
		expect(TimeUtils.departureTimeToStr((TimeUtils.currentTimeInMinutes() + 5)*60))
			.toMatch(/5\smin/);
	...
	it('shows departure time, if departure not in next ten minutes', () => {
		expect(TimeUtils.departureTimeToStr((TimeUtils.currentTimeInMinutes() + 10)*60))
			.toMatch(/[0-9]{1,2}.[0-9]{2,2}/);
		expect(TimeUtils.departureTimeToStr((TimeUtils.currentTimeInMinutes() - 3)*60))
			.toMatch(/[0-9]{1,2}.[0-9]{2,2}/);

		const mock = jest.spyOn(TimeUtils, 'currentTimeInMinutes').mockImplementation(() => 23*60+59);
		expect(TimeUtils.departureTimeToStr((TimeUtils.currentTimeInMinutes() + 11)*60))
			.toMatch(/[0-9]{1,2}.[0-9]{2,2}/);

		mock.mockImplementation(() => 24*60);
		expect(TimeUtils.departureTimeToStr((TimeUtils.currentTimeInMinutes())*60 - 3))
			.toMatch(/[0-9]{1,2}.[0-9]{2,2}/);
	});
});
...
describe('getUniqueFilter', () => {
	it('can be used to filter array to have unique member only', () => {
		...
		expect(arr.filter(getUniqueFilter('a[0]'))).toEqual([{a:[0]}, {a:[1]}]);
	});
});
