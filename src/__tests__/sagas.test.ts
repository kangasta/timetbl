...
describe('getData', (): void => {
	it('sends an API query', (): void => {
		[
			...
			const sendQuery = jest.spyOn(ApiUtils, 'sendQuery').mockImplementation(() => Promise.resolve([]));
			const dispatch = jest.fn();
			...
			expect(dispatch).toHaveBeenCalledWith({type: 'NEW_DATA', metadata: {type, data: []}});
		});
	});
	it('returns error when no stops given for stopDepartures', async () => {
		...
		expect(dispatch).toHaveBeenCalledWith({type: 'NEW_DATA', metadata: {type, data: [], loading: null, error: 'No stops given'}});
	});
});
