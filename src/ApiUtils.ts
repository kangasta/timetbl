...
const nearestQuery = (
  placeType: string,
  placeQuery: string,
  { lat, lon, maxDistance, maxResults }: PositionParameters
): string => `
	{ nearest(
		lat: ${lat},
		lon: ${lon},
...
const nearestBikesQuery = (parameters: PositionParameters): string =>
  nearestQuery(
    'BICYCLE_RENT',
...
const nearestDeparturesQuery = (parameters: PositionParameters): string =>
  nearestQuery(
    'DEPARTURE_ROW',
    `... on DepartureRow {
		stoptimes (
...
const nearestStopsQuery = (parameters: PositionParameters): string =>
  nearestQuery(
    'STOP',
    `... on Stop {
		${stopQuery}
...
const sortStoptimesData = (
  data: StoptimesSortable[],
  isNearestNode: boolean
): StoptimesSortable[] => {
  const getStoptimes = (item: StoptimesSortable): StoptimeData[] =>
    isNearestNode
      ? (item as NearestNode<StoptimesData>).node.place.stoptimes
      : (item as StoptimesData).stoptimes;

  return data
    .filter((item: StoptimesSortable): boolean => getStoptimes(item).length > 0)
    .sort((a: StoptimesSortable, b: StoptimesSortable): number => {
  ...
  const promises = parameters.map(
    async (parametersItem): Promise<object> => {
      ...
      if ('errors' in jsonData) {
        throw new Error(
          'HSL API returned object with errors content instead of data:\n' +
